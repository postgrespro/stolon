...
func (p *PostgresKeeper) postgresKeeperSM(pctx context.Context) {
	...
	host, _ := os.Hostname()
	log := &TeeLogger{pctx : pctx, p: p, host: host, base: log.Desugar().WithOptions(zap.AddCallerSkip(1)).Sugar() }
	...
	if dbls.Initializing {
		...
		log.Warnw("db failed to initialize or resync") // 'Warn' instead of 'Error' cause 'Error' will be overwritten by next "real" error
	...
	if p.dbLocalState.UID != db.UID {
		...
		case cluster.DBInitModePITR:
			...
			if err = pgm.StopIfStarted(true); err != nil {
				log.Fatalw("failed to stop pg instance", zap.Error(err))
				return
			}
			if err = pgm.RemoveAll(); err != nil {
				log.Fatalw("failed to remove the postgres data dir", zap.Error(err))
				return
			}
			log.Infow("executing DataRestoreCommand")
			if err = pgm.Restore(db.Spec.PITRConfig.DataRestoreCommand); err != nil {
				log.Fatalw("failed to restore postgres database cluster", zap.Error(err))
				return
			}
			...
			if err = pgm.StartTmpMerged(); err != nil {
				log.Fatalw("failed to start instance", zap.Error(err))
				return
			}
			...
			if recoveryMode == pg.RecoveryModeRecovery {
				...
				if err = pgm.WaitRecoveryDone(cd.Cluster.DefSpec().SyncTimeout.Duration); err != nil {
					log.Fatalw("recovery not finished", zap.Error(err))
					return
				}
			...
			if err = pgm.WaitReady(cd.Cluster.DefSpec().SyncTimeout.Duration); err != nil {
				log.Fatalw("timeout waiting for instance to be ready", zap.Error(err))
				return
			}
			...
			if db.Spec.IncludeConfig {
				pgParameters, err = pgm.GetConfigFilePGParameters()
				if err != nil {
					log.Fatalw("failed to retrieve postgres parameters", zap.Error(err))
					return
				}
				ndbls.InitPGParameters = pgParameters
				if err = p.saveDBLocalState(ndbls); err != nil {
					log.Fatalw("failed to save db local state", zap.Error(err))
					return
				}
			}
